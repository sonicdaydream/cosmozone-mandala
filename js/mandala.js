...
/* ============================================================
   コスモゾーン — 無限入れ子曼荼羅
   各層は「中心が空洞のリング」。空洞の中に次の層が入る。
   リングの内外半径比 = K なので、層はすき間なく平面を埋める。
   ズームが1層ぶん進んだら外側の層を捨て、内側に新しい層を産む。
   → 画面に出るのは常に5〜6層。深さが増えても計算量も精度も変わらない。

   window.MANDALA_CONFIG.mode で描画内容を切り替える:
     'abstract' … 抽象の書字・幾何紋様のみ（尊像・梵字なし）
     'bonji'    … 五智如来を梵字（種子）で表示
     'butsu'    … 五智如来を坐像で表示
   ============================================================ */

const MODE = (window.MANDALA_CONFIG && window.MANDALA_CONFIG.mode) || 'butsu';
document.body.classList.add('mode-' + MODE);

const K = 4;                       // 1層あたりの倍率
const U_MIN = -50, U_MAX = 70;     // 120層で一周して元に戻る
const PERIOD = U_MAX - U_MIN;      // 120。8の倍数（曲の区間）かつ3の倍数（音の重ね数）
...
const HI = lowMem ? 1024 : 1600;   // 画面を占める層のテクスチャ解像度
const LO = lowMem ?  640 : 1024;   // 奥の小さい層用
...
function resize(){
  ...
  D = 0.5 * Math.hypot(W, H) * 1.02;   // 画面対角の半分＝基準層の外半径
}
addEventListener('resize', resize);
resize();

/* ── 乱数（層番号から決まる。同じ層は何度捨てても同じ絵） ── */
function hash32(x){
  x |= 0;
...
/* ============================================================
   1層ぶんの曼荼羅を描く
   実際の曼荼羅の構成に寄せる：
     内院＝蓮弁 → 月輪に坐す尊（or 種子・抽象紋） → 楼閣（四門） → 火焔・雲・金剛杵
   中尊の座にあたる中心は空洞で、そこに次の曼荼羅が入る。
   ============================================================ */
const PIGMENT = [
  { h:222, s:60 },  // 群青
  { h:  8, s:72 },  // 朱
...
/* 五智如来の種子（Unicode 悉曇領域。符号位置は一字ずつ照合済み） */
const SEEDS = [
...
/* ── 抽象紋（尊像・梵字を使わない版のための幾何マーク） ── */
function drawAbstractMark(g, s, C, idx){
  g.save();
  g.rotate((idx % 8) * Math.PI/4 + Math.PI/8);
  const n = 6, r0 = s*0.17, r1 = s*0.40;
  g.beginPath();
  for(let k=0;k<n*2;k++){
    const a = k*Math.PI/n;
    const r = (k%2===0) ? r1 : r0;
  ...
  g.beginPath(); g.arc(0, 0, r0*0.55, 0, Math.PI*2);
  g.fillStyle = C.gold; g.fill();
  g.restore();
}
...
/* 四方に四仏、四隅に副次要素。MODEに応じて坐像／種子／抽象紋を出し分ける。
   中央（大日）の座は次の曼荼羅そのもの */
function bandFiveWisdom(g, r0, r1, C){
  ...
  for(const S of SEEDS){
    ...
    if(MODE === 'abstract'){
      ...
      drawAbstractMark(g, disc*1.5, C, idx);
    }else if(MODE === 'butsu'){
      g.rotate(S.ang + Math.PI/2);
      drawFigure(g, disc*1.9, C, idx);
    }else if(bonjiReady){
      drawBonji(g, S.ch, disc*1.26, C);
    }else{
      g.rotate(S.ang + Math.PI/2); drawSeed(g, disc*1.45, C, idx);
    }
    g.restore();
    idx++;
  }
  for(let k=0;k<4;k++){
    ...
    if(MODE === 'abstract')   drawAbstractMark(g, d2*1.6, C, k+5);
    else if(MODE === 'bonji') drawSeed(g, d2*1.45, C, k+5);
    else                      drawFigure(g, d2*1.72, C, k);
    g.restore();
  }
}
...
function paintOctave(S, i, seed){
  const cvs = document.createElement('canvas');
  cvs.width = cvs.height = S;
  const g = cvs.getContext('2d');
  const R = S/2, RIN = R/K, lw = Math.max(1, S/620);
  ...
  bandFiveWisdom(g, e[1]*1.01, e[2]*.99, C);
...
function texture(i, hi, budget){
  ...
  let e = cache.get(key);
  if(e) return e;
  const other = cache.get(w + ':' + (hi ? LO : HI));
  if(other && budget.used >= budget.max) return other;   // 今フレームは間に合わせで代用
  if(budget.used >= budget.max) return null;
  budget.used++;
  e = paintOctave(res, w, seed);
  cache.set(key, e);
  return e;
}
function prune(u){
  const cur = wrapI(Math.round(u));
  for(const key of Array.from(cache.keys())){
    let d = Math.abs(parseInt(key, 10) - cur) % PERIOD;
...
/* ============================================================
   共有URL（?s=種・?u=深さ）から状態を復元する
   ============================================================ */
const qs = new URLSearchParams(location.search);
const qSeed = qs.get('s'), qU = qs.get('u');
const openedFromShare = qSeed !== null && qU !== null;

/* ============================================================
   状態
   ============================================================ */
let u = openedFromShare ? Math.max(U_MIN, Math.min(U_MAX, parseFloat(qU))) : 0;   // 連続的な層位置。増える＝内へ潜る
let laps = 0;              // 何周したか
let seed = openedFromShare ? (parseInt(qSeed, 10) | 0) : (Math.random()*1e9)|0;
let playing = !openedFromShare;
let dir = 1;
let speed = 1 / (32 * (60/138/4));   // 層／秒。1層＝ちょうど2小節
...
const rotSpeed = () => 0;   // 方位を保つため層は回さない
...
function nameOf(e){
  for(const [lim, nm] of STEPS){ if(e >= lim) return nm; }
  return 'プランク長の彼方';
}
...
/* ============================================================
   音 — 無限下降トランス
   骨格は映像と同じ：オクターブ違いのノコギリ波を6本重ね、下端に
   落ちた音を消して同時に上端へ足す。だから永久に下がり続ける。
   さらに 8層ぶんを1周期とする構成（沈む→脈→溜め→放出）を持たせ、
   層の位置がそのまま曲の進行になるようにした。
   止めれば、その区間が永遠にループする。
   ============================================================ */
const OCT_PER_LAYER = Math.log2(K);      // 1層 = K倍 = 2オクターブ
const NPART = 6;
const FMIN  = 32.703;                     // C1
const BPM   = 138;                        // トランスの標準
const STEP  = 60 / BPM / 4;               // 16分音符（秒）
const BEAT  = 60 / BPM;
const ROOTS = [0, 3, 5, 7, 8, 10];        // Cマイナーの音度（半音）
const ARP   = [0,7,12,7,3,10,15,10, 0,7,12,15,12,7,3,7];

// 4つの区間。1層 = 2小節、8層で1周
const SECTIONS = [
  { sw:.10, chord:.60, sub:.50, cutLo:180, cutHi: 800 },   // 0 沈む
  { sw:.18, chord:.34, sub:.62, cutLo:260, cutHi:2200 },   // 1 脈
  { sw:.44, chord:.28, sub:.44, cutLo:380, cutHi:7500 },   // 2 溜め
  { sw:.26, chord:.62, sub:.78, cutLo:700, cutHi:4200 }    // 3 放出
];
...
function initAudio(){
  ...
  const dl    = ac.createDelay(1.2); dl.delayTime.value = BEAT * 0.75;   // 付点8分
  ...
  // コード：Cマイナーを固定で鳴らし続ける。和音の色はベースの根音で変わる
  ...
  return { ac, master, limiter, send, dlSend, duck,
           swFilter, swGain, parts, chordGain, bassGain, bSub, bHar,
           noise: makeNoise(ac) };
}

/* ── 毎フレーム：層の位置から音程・フィルター・区間を決める ── */
...
function updateAudio(){
  ...
  for(let j=0;j<NPART;j++){
    ...
    const a = (0.5 - 0.5*Math.cos(2*Math.PI * x / NPART)) * 0.085;   // 上下端で必ず無音
    const P = A.parts[j];
    P.oscs[0].frequency.setTargetAtTime(f, now, .03);
    P.oscs[1].frequency.setTargetAtTime(f, now, .03);
    P.g.gain.setTargetAtTime(a, now, .05);
  }
...
/* ============================================================
   計測（Vercel Web Analytics のカスタムイベント）
   stay / dive / share。カスタムイベントは Analytics が Pro プラン
   以上の場合のみダッシュボードに表示される。
   ============================================================ */
function track(name){
  try{ if(window.va) window.va('event', { name }); }catch(e){}
}
setTimeout(() => track('stay'), 30000);
let divedOnce = false;
...
document.getElementById('btnSeed').onclick = () => {
  seed = (Math.random()*1e9)|0; cache.clear();
...
/* ── 共有URLを組み立てる（?s=種・?u=深さ） ── */
function shareUrl(){
  const url = new URL(location.href);
  url.search = '';
  url.searchParams.set('s', String(seed));
  url.searchParams.set('u', u.toFixed(3));
  return url.toString();
}
async function doShare(){
  const link = shareUrl();
  history.replaceState(null, '', '?s=' + seed + '&u=' + u.toFixed(3));
  const shareText = '無限に潜れる曼荼羅 — ' + elVal.textContent + 'まで潜った';
  try{
    if(navigator.share){
      await navigator.share({ title: document.title, text: shareText, url: link });
      setRecordStatus('共有しました');
    }else{
      await navigator.clipboard.writeText(link);
      setRecordStatus('リンクをコピーしました');
    }
  }catch(e){
    try{
      await navigator.clipboard.writeText(link);
...
function record15s(){
  if(!cv.captureStream || typeof MediaRecorder === 'undefined'){
    setRecordStatus('この端末では録画に対応していません');
    return;
  }
  ...
  rec.onstop = () => {
    ...
    a.download = 'cosmozone_' + seed + '_' + u.toFixed(2) + '.' + ext;
    a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href), 4000);
    setRecordStatus('録画を保存しました');
  };
  setRecordStatus('録画中…（15秒）');
  rec.start();
  setTimeout(() => { if(rec.state !== 'inactive') rec.stop(); }, 15000);
}
...
function setRecordStatus(msg){ if(recordStatus) recordStatus.textContent = msg; }
if(btnRecord && recordPanel){
  btnRecord.onclick = () => { recordPanel.hidden = !recordPanel.hidden; };
  const btnShareLink = document.getElementById('btnShareLink');
  const btnRec15 = document.getElementById('btnRec15');
  const btnSaveImg = document.getElementById('btnSaveImg');
  if(btnShareLink) btnShareLink.onclick = doShare;
  if(btnRec15) btnRec15.onclick = record15s;
  if(btnSaveImg) btnSaveImg.onclick = saveImage;
}

function takeControl(){
  if(playing){ playing = false; btnPlay.textContent = '再生'; }
  if(!divedOnce){ divedOnce = true; track('dive'); }
}
function zoomBy(dU){
  const nu = u + dU;
  if(nu > U_MAX) laps++; else if(nu < U_MIN) laps--;
...
/* ── 回して潜る（時計回り＝内へ、反時計回り＝外へ） ── */
const TURN_LAYERS = 1.0;   // 一回転で1層＝4倍
const DEAD_R = 46;         // 中心付近は角度が暴れるので触らない
let rot = null;            // 指を置いているあいだの状態
let spin = 0;              // 離したあとの惰性（層／秒）
...
addEventListener('pointermove', e => {
  ...
  zoomBy(dU);
  rot.vel = rot.vel*0.72 + (dU/dt)*0.28;   // 直近の角速度をならす
});
function endRot(e){
...
if(MODE === 'bonji'){
  ...
  // abstract: 尊名の説明は不要
  if(btnNames) btnNames.remove();
  if(legend) legend.remove();
  const wn = document.getElementById('warn'); if(wn) wn.remove();
}

/* バージョン切り替えナビの現在地表示 */
document.querySelectorAll('#verNav a').forEach(a => {
  if(a.dataset.m === MODE) a.classList.add('current');
});
